...
export class CompositionHandler extends Handler {
  ...
  async executeFunction(args: { [key: string]: any[] }, options: CompositionHandlerOptions):
    ...
    for (const id of options.dependencyList) {
      ...
      const implementationId = options.implementationHandler
        .getImplementations(options.dependencyMeta[id].fn.id)[0];
      const thisArgs = {};
      for (const predicate in options.dependencyMeta[id].inputs) {
        thisArgs[predicate] = options
          .composedOfValueMap[options.dependencyMeta[id].inputs[predicate][0]];
      }
      const results = await options.implementationHandler
        .executeImplementation(implementationId, thisArgs);
